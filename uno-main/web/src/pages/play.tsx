...
import { SERVER_HTTP_URL, SERVER_WS_URL } from '../config/server';
...
function Play(): React.ReactElement {
  ...
  // Share link with room and backend endpoints, so guests connect to the same server
  const api = encodeURIComponent(SERVER_HTTP_URL);
  const ws = encodeURIComponent(SERVER_WS_URL);
  const url = `${WEB_HTTP_URL}?join=${encodeURIComponent(config.room)}&api=${api}&ws=${ws}`;
